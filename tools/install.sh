...
# files across FAT volumes, and CircuitPython's App.load_all (and plain
# `import`) chokes trying to load `._code.py` etc. So this copies with
# rsync's --exclude, then sweeps the destination for any that snuck in
# anyway (Finder can add them back just from the volume being open).
set -euo pipefail
